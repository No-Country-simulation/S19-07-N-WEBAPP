...
export function Sidebar({ className }: SidebarProps) {
    ...
    return (
        <div className={cn("pb-12 min-h-screen bg-[#2c3e50]", className, { 'w-16': isCollapsed, 'w-64': !isCollapsed })}>
            <div className="space-y-4 py-4">
                ...
                <nav className="space-y-1 px-2">
                    ...
                    <NavItem icon={Building2} text={isCollapsed ? '' : 'Sucursales'} to="/places" />
                    <NavItem icon={Users} text={isCollapsed ? '' : 'Personas'} to="/users" />
                    <NavItem icon={PieChart} text={isCollapsed ? '' : 'Finanzas'} to="/finance" />
                </nav>
            </div>
